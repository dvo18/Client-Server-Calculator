        ...
        try:
            ...
            try:
                if p1.f !=None:
                    ...
                    elif p2.v !=None: r.v = [ x+p1.f for x in p2.v]                     # comprensión de listas
                    ...
                    try:
                        if c(p1.m):
                            if p2.f !=None: r.m = [ [ c+p2.f for c in f ] for f in p1.m ]
                                ...
                                try:
                                    if c(p2.m):
                                        if len(p1.m)==len(p2.m) and len(p1.m[0])==len(p2.m[0]): r.m = [ [ p1.m[i][j]+p2.m[i][j] for j in range(len(p1.m[0])) ] for i in range(len(p1.m)) ]
                                        else: raise ValueError
